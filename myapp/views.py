...
def book_machinery(request):
    if request.method == "POST":
        ...
        bdate_datetime = datetime.datetime.strptime(bdate, '%Y-%m-%d')
...
def auth_register(request):
    if request.method == "POST":
        ...
        contact = data.get('phone',None)
        addresss = data.get('address',None)
        print(first_name,email,contact)
        if customer.objects.filter(first_name=first_name).exists():
            return JsonResponse({'error': 'FirstName already exists'}, status=405)
        if customer.objects.filter(last_name=first_name).exists():
            return JsonResponse({'error': 'LastName already exists'}, status=405)
        ...
        if customer.objects.filter(contact=contact).exists():
            return JsonResponse({'error': 'Contact already exists'}, status=405)
        print("1")
        new_customer = customer(first_name=first_name,last_name=last_name,email=email, contact=contact, address=addresss)
        new_customer.save()
        return JsonResponse({'msg': 'User Registered',"customer":new_customer.id}, status=200)
...
def auth_login(request):
    if request.method == "POST":
        ...
        if customer.objects.filter(email=email).exists():
            ...
            print("in check otp is :", otp)
            request.session['otp_data'] = otp

            request.session.save()
            print(request.session['otp_data'])
            cache.set('otp', otp)
            # otp_session.append(otp)
            # print("in check email",otp_session)
            # print("in session the store otp is :",request.session["otp"])
            subject = 'About Login...'
            message = (f"Hey, your login process is started on Revolutioninzing Agriculture and your OTP is: {otp}. Your OTP is valid for 5 minutes only.")
            email_from = 'karinapatel615@gmail.com'
            cus = [email]
            print(otp)
            sent = send_mail(subject, message, email_from, cus)
            if sent:
                print("Session keys:", request.session.keys())
                get_customer = customer.objects.get(email=email)
                return JsonResponse({'msg': 'Otp Sent','customer':get_customer.id}, status=200)
            else:
                return JsonResponse({'error': 'otp not sent'}, status=405)
        else:
            return JsonResponse({'error': 'User not found, please first register'}, status=405)
    return JsonResponse({'error': 'Method not allowed'}, status=401)
...
def auth_otp(request):
    if request.method == "POST":
        ...
        # Check if 'otp_data' exists in the session
        session_otp = request.session.get('otp_data')
        ...
        if otp_s is None:
            print("OTP session data not found")
            return JsonResponse({'error': 'OTP session data not found'}, status=400)
        ...
        if int(otp) == otp_s:
            ...
            # request.session.save()
            cusId = cache.get('cusId')
            return JsonResponse({'msg': cusId}, status=200)
        else:
            return JsonResponse({'error': 'Otp not valid'}, status=405)
...
def update_profile(request, id):
    if request.method == "POST":
        ...
        add = request.POST.get("address")
        try:
            getcustomer = customer.objects.get(email=email)
            # print(id)
            print(getcustomer)
            if getcustomer is None:
                return JsonResponse({"error": "Session is empty. Please sign in."}, status=401)
            # if request.session.is_empty():
            #     return JsonResponse({"error": "Session is empty. Please sign in."}, status=401)
            else:
